"""Map the Pareto frontier in ANY band between two integer bit-widths.

Generalises lowbit_pareto.py. The frontier argument is structural: uniform
quantization only exists at integer bits, so between any B and B+1 the only
options are mixed precision (a few discrete points) or patches (continuous).
This script tests one band at a time -- set SQ_BASE_BITS=B and SQ_REF_BITS=B+1.

Run so far:  B=3 (full_surface.py, patches own 4/5 frontier points)
             B=2 (lowbit_pareto.py, more headroom but floor risk)
             B=4 (this, "less quantization" -- the 4.25->5.25 band)
...
OUT_PATH = os.environ.get("SQ_OUT", "results/band_pareto.json")
...
BASE_B = int(os.environ.get("SQ_BASE_BITS", "4"))
REF_B = int(os.environ.get("SQ_REF_BITS", str(BASE_B + 1)))
PATCH_BITS = tuple(int(x) for x in os.environ.get("SQ_PATCH_BITS", "3,4").split(","))
...
def main():
    ...
    qc = {}
    for b in (BASE_B, REF_B):
        qc[b] = {n: gptq_quantize(w_orig[n], H_pool[n], bits=b, group_size=GROUP_SIZE)[0]
                 for n in order}
        print(f"  {b}-bit", flush=True)

    base_defs = {
        f"u{BASE_B}": lambda n: BASE_B,
        f"mix{REF_B}{BASE_B}": lambda n: REF_B if is_attn(n) else BASE_B,
    }
    ...
    for t in TASKS:
        ...
        score(f"uniform{BASE_B}bit", qc[BASE_B], bpw(lambda n: BASE_B))
        score(f"uniform{REF_B}bit", qc[REF_B], bpw(lambda n: REF_B))
        score(f"mixed_attn{REF_B}_mlp{BASE_B}", base_w[f"mix{REF_B}{BASE_B}"],
              bpw(base_defs[f"mix{REF_B}{BASE_B}"]))

        for btag in (f"u{BASE_B}", f"mix{REF_B}{BASE_B}"):
            obs = {}
            for n in order:
                Hg = H_t[n].to(dev)
        ...
        sig = {}
        for k in row:
            if k in ("full_precision", f"uniform{REF_B}bit"):
                continue
            if bits[k] <= bits[f"uniform{REF_B}bit"]:
                sig[f"{k}_beats_uniform{REF_B}"] = boot(cache[k], cache[f"uniform{REF_B}bit"])
        results["ppl"][t] = row
        results["bits"][t] = bits
        results["sig"][t] = sig
