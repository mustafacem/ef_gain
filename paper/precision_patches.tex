...
patch \emph{mechanism} is real: task-matched patches beat per-layer-type mixed
...
$2.13\times$. \textbf{(3)} Nevertheless, uniform quantization remains the
...
but it does not beat the simplest baseline---spending the same bits
uniformly---on absolute quality. Its genuine value is narrow and geometric:
integer-only uniform quantization cannot occupy the memory band between
consecutive bit-widths, and patches (together with mixed precision) fill it.
