...
perplexity and GSM8K accuracy. \textbf{(5)} Replacing restoration with an
end-to-end \emph{learned} (distillation-trained) patch breaks a real ceiling:
it beats the restored patch on both tasks and closes up to 66\% of the
base$\to$uniform-4 gap, though it still does not surpass uniform-4's absolute
quality, and it moves the method from training-free to QLoRA-adjacent. Finally, we report a methodological finding
...
distinction is that patches add no parameters and store a quantized residual
rather than a learned low-rank update. We deliberately avoid learned patches to
keep the comparison to uniform quantization clean.
...
\paragraph{Models and surface.} Qwen2.5-Instruct at 0.5B, 1.5B, and 7B. Unless
stated, experiments quantize the \emph{full surface}: all seven linear
projection types (\code{q,k,v,o,gate,up,down}), 358M weights for the 0.5B
model. Prior iterations that quantized only \code{o\_proj}$+$\code{down\_proj}
(35\% of weights) left the rest in \code{fp16} and thereby inflated quality;
all headline numbers here use the full surface.
...
\label{sec:frontier}
Table~\ref{tab:frontier} reports GSM8K accuracy across configurations spanning
2.25--5.25 \bpw{}. Three regimes emerge.
...
\textbf{At the integer rungs, uniform is competitive.} \code{mix43+pb3}
(4.198 \bpw{}) reaches 23.0\% versus uniform 4-bit's 23.5\% (4.250 \bpw{})---a
statistical tie ($p{=}0.406$ by paired bootstrap) at lower memory. The same
holds in perplexity ($p{=}0.406$), so the parity claim is confirmed under two
independent metrics.
...
\section{Learned patches partially break the restoration ceiling}
\label{sec:learned}
...
patch can at best claw the quantized model back toward fp16, group by group.
This is a hard ceiling. A patch \emph{trained} end-to-end against the fp16
teacher can, in principle, do something restoration cannot: compensate for
quantization error that propagates \emph{across} layers, since the loss is
computed on the final logits rather than one layer's output. We test whether
...
\emph{worse}, consistent with Sec.~\ref{sec:negative}). The residual is trained
for 4 epochs with a distillation loss $\mathrm{KL}(p_\text{fp16}\,\|\,p_\text{student})$
on task calibration text, then \emph{quantized to 3 bits} and stored exactly
like a solved patch, at identical bytes. This is deliberately the simplest
learned variant---no task labels, no hidden-state matching, no joint
optimization of base and patch---so any gain is attributable to end-to-end
training alone.

\paragraph{Result.} Table~\ref{tab:learned} compares the learned patch against
the solved (restoration) patch at matched memory (3.43 \bpw{}).

\begin{table}[t]
\centering
\caption{Learned vs.\ solved patches (perplexity, matched 3.43 \bpw{}). The
learned patch beats restoration on both tasks and, on math, turns a patch that
\emph{hurt} (solved $5.97 >$ base $5.85$) into one that closes 66\% of the
base$\to$uniform-4 gap. It still does not reach uniform-4's absolute quality.}
\label{tab:learned}
\begin{tabular}{lrrrrr}
\toprule
task & base & solved & \textbf{learned} & uniform-4 & fp16 \\
     & (3.37b) & (3.43b) & \textbf{(3.43b)} & (4.25b) & \\
\midrule
math & 5.85 & 5.97 & \textbf{5.11} & 4.73 & 4.49 \\
code & 8.02 & 7.87 & \textbf{7.67} & 5.47 & 4.94 \\
\bottomrule
\end{tabular}
\end{table}

Four observations. \textbf{(1)} Learning beats restoration on both tasks
($+0.86$ ppl on math, $+0.20$ on code); on math the solved patch was actively
harmful while the learned patch closes 66\% of the gap to uniform-4.
\textbf{(2)} The benefit is strongly task-dependent (66\% vs.\ 14\% of the gap),
mirroring where task conditioning was strongest in Sec.~\ref{sec:atlas}.
\textbf{(3)} Quantizing the trained delta to 3 bits is free: the 3-bit and
fp16 learned patches differ by $<0.02$ ppl, refuting the concern that a
continuously-trained correction cannot be stored cheaply. \textbf{(4)}
Nevertheless, the learned patch still does not beat uniform-4 on absolute
quality ($5.11$ vs.\ $4.73$; $7.67$ vs.\ $5.47$). The frontier verdict stands;
what changes is that a \emph{learned} patch is a materially better point on the
sub-integer-bit frontier than a \emph{restored} one.

\paragraph{Scope.} This shifts the problem setting from training-free PTQ to a
QLoRA-adjacent trained residual; the comparison to uniform quantization remains
fair (matched bits), but ``no additional training'' is no longer a property of
the method. The result is two tasks, single seed, 0.5B, \code{down\_proj} only,
under a 6\,GB training budget. It establishes that end-to-end learning escapes
the restoration ceiling; whether the richer directions it opens---joint
base/patch optimization, activation quantization, or input-conditional
precision, none of which we tested---can close the remaining gap to uniform-4
is left open.
...
Config ordering is identical under both metrics, so \emph{comparative}
conclusions in this paper (and in PTQ work generally) are safe. But absolute
``gap closed'' or ``$X\%$ recovered'' figures reported in perplexity---ubiquitous
in the quantization literature---overstate how much task capability survives.
We recommend downstream accuracy as a primary metric for sub-4-bit claims.
...
quantization cannot occupy, and a mixed-precision base plus a small 3-bit patch
ties uniform 4-bit at lower memory on both perplexity and GSM8K accuracy. The
robust, deployable win is the mixed-precision base itself. Equally important
are the guardrails the study produced: a noise-ceiling methodology for
task-sensitivity claims, a demonstration that perplexity understates
quantization damage by roughly an order of magnitude, and a quantification of
calibration-seed variance that retroactively bounds which small ``wins'' in
this space are real. We release everything, including the negative results, in
the hope that the matched-memory, noise-ceiling, accuracy-first discipline is
as useful as any single number.
